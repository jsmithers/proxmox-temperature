...
if ! grep -q "thermalstate" /usr/share/perl5/PVE/API2/Nodes.pm; then
  ...
  echo "'Nodes.pm' is backed up to '/usr/share/perl5/PVE/API2/Nodes.pm.backup-${CURR_DATE}'"
...
if [ ! -z "${line}" ]; then
  ...
  echo "'pvemanagerlib.js' is backed up to '/usr/share/pve-manager/js/pvemanagerlib.js.backup-${CURR_DATE}'"
...
if ! grep -q "sensors-format" /usr/share/perl5/PVE/Service/pvestatd.pm; then
  ...
  echo "'pvestatd.pm' is backed up to '/usr/share/perl5/PVE/Service/pvestatd.pm.backup-${CURR_DATE}'"
...
echo "CPU temperature feature implemented. I you have issues try restoring backup files."
